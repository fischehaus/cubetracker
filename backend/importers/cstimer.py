...
def _to_naive_utc(ts: datetime) -> datetime:
    """Normalisiert datetime auf naive UTC fuer SQLite-Vergleiche.

    SQLite speichert datetime ohne tzinfo zurueck, daher muessen wir
    aware-datetimes auf naive UTC konvertieren, bevor wir sie mit
    geladenen Werten vergleichen.
    """
...
def import_cstimer_json(payload: dict[str, Any], db: OrmSession) -> ImportResult:
    """Hauptfunktion: csTimer-JSON-Daten in die DB importieren.

    Idempotent: Bei Re-Import werden Sessions per cstimer_session_id
    ...
    # Sammle existierende Solves pro Session als Set fuer schnelle Duplikat-Pruefung
    # (wird gefuellt, sobald Session existiert)
    ...
    for key, solve_entries in payload.items():
        ...
        # Nur Sessions mit Solves importieren
        ...
        # Session per cstimer_session_id finden oder anlegen
        existing = db.scalar(select(DbSession).where(DbSession.cstimer_session_id == cstimer_sid))
        if existing is None:
            session = DbSession(
                name=session_name,
            ...
            db.add(session)
            db.flush()  # damit wir session.id haben
            result.sessions_created += 1
        else:
            # Update-Felder bei Bedarf
            updated = False
        ...
        # SQLite speichert datetime ohne tzinfo zurueck — wir normalisieren auf
        # naive UTC fuer den Vergleich.
        if session.id not in existing_solve_keys_per_session:
            keys: set[tuple[datetime, int]] = {
                (_to_naive_utc(ts), tm)
                for ts, tm in db.execute(
                    select(Solve.timestamp, Solve.time_ms).where(Solve.session_id == session.id)
                ).all()
            }
        ...
        for entry in solve_entries:
            ...
            dedup_key = (_to_naive_utc(parsed["timestamp"]), parsed["time_ms"])

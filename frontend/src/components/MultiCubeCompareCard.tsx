...
function CubeRow({ cube }: { cube: CubeStats }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 py-3 border-b border-gray-800/60 last:border-0">
      <div className="min-w-0 flex-1">
        <div className="text-base text-gray-100 font-medium">{cube.cube_type}</div>
        ...
        {improvement && (
          <div
            className={`text-xs mt-0.5 ${improvement.color}`}
      ...
      <div className={`text-right ${form.color} shrink-0`} title={tooltip}>
        <div className="text-base font-mono font-semibold">
          ...
          {!useRecent && factor !== null && (
            <span className="ml-1 text-[10px] text-gray-500 align-top">life</span>
          )}
        </div>
        <div className="text-xs text-gray-500">{cube.count_valid} Solves</div>
      </div>
    </div>
  );
}
  ...
  if (!data || data.cubes.length === 0) {
    return (
      <div className="rounded-lg border border-gray-700 bg-gray-900/50 p-6">
        <h2 className="text-2xl font-semibold text-gray-100 mb-2">Cube-Vergleich</h2>
  ...
  return (
    <div className="rounded-lg border border-gray-700 bg-gray-900/50 p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-2xl font-semibold text-gray-100">Cube-Vergleich</h2>
        <span className="text-sm text-gray-500">aktuelle Form</span>
      </div>

...
export function SolveDetailModal({ solve, ao5, ao12, isPb, onClose }: Props) {
  ...
  return (
    <>
        ...
        >
          ...
          <dl className="space-y-2 mb-5 text-sm">
            ...
            <MetaRow
              ...
              value={
                hardwareName ?? <span className="text-gray-600 italic">—</span>
              }
            />
            <MetaRow
              label={t("solveDetail.sessionLabel")}
              value={
                sessionName ?? <span className="text-gray-600 italic">—</span>
              }
            />
            <MetaRow
          ...
          <div className="flex gap-2 flex-wrap pt-3 border-t border-gray-800">
            ...
            >
              {solve.dnf ? t("solveDetail.dnfRemove") : t("solveDetail.dnfSet")}
            </button>
            <button
      ...
      {confirmDelete && (
        <ConfirmDialog
          message={t("solveDetail.deleteConfirm", {
            time: formatSolveTime(solve),
          })}
          busy={del.isPending}
          onConfirm={() => del.mutate(solve.id, { onSuccess: onClose })}
